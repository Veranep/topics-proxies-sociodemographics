...
class AbstractModel(ABC):
    ...
    @abstractmethod
    def generate(
        self, prompt, max_new_tokens=100, temperature=0.1, do_sample=False
    ):
        pass
...
class TransformerLensModel(AbstractModel):
    def generate(
        self, prompt, max_new_tokens=50, temperature=0.1, do_sample=False
    ):
    ...
    def wrap_prompt(self, prompt: str) -> str:
        if self.is_it():
            s = self.model.tokenizer.apply_chat_template(
...
class OpenEndedQuestion(DataClassJsonMixin):
    ...
    def parse_answer(self, answer: str, it=True) -> str:
        if it:
            ...
            match = re.match(
                r"(Q: .*?A: .*?){3}Q: .*?A: (.*?)(Q|\Z)", answer, re.DOTALL
            )
...
class EvaluationResult:
    ...
    def __post_init__(self):
        self.score_from_valid = (
            self.correct / self.valid if self.valid > 0 else 0
        )
...
class OpenEndedEvaluationResult:
    ...
    def __post_init__(self):
        ...
        self.coherence_score = (
            np.mean(self.coherent) if self.coherent else None
        )
...
"""
    ...
    def evaluate_familiarity(
        self, prompts: list[str], completions: list[str]
    ) -> tuple[list[int], float, list[tuple[str, str]]]:
        counter = 0
        scores = []
        for prompt, completion in zip(prompts, completions):
            the_prompt = self.FAMILIARITY_PROMPT.format(
    ...
    def evaluate_open_ended_questions(
        self, questions: list[tuple[OpenEndedQuestion, str]], quit_thresh=None
        ...
        for question, attempted_answer in questions:
            prompt = self.OPEN_QA_PROMPT.format(
        ...
        return OpenEndedEvaluationResult(
            corrects, total, qas, responses, coherent, quit
        )
    ...
    def evaluate_open_ended_questions_no_coherence(
        self, questions: list[tuple[OpenEndedQuestion, str]], quit_thresh=None
    ) -> OpenEndedEvaluationResult:
        corrects = 0
        total = len(questions)
        ...
        for question, attempted_answer in questions:
            prompt = self.OPEN_QA_PROMPT_NO_COHERENCE.format(
                question=question.question,
                answer=question.answer,
                attempted_answer=attempted_answer,
            )
        ...
        return OpenEndedEvaluationResult(
            corrects, total, qas, responses, quit=quit
        )
    ...
    def evaluate_alpaca_instruct_score(
        self, instructions: str, completion: str
    ) -> tuple[int, str]:
...
[Completion End]"""
    ...
    def evaluate_alpaca_fluency_score(
        self, completion: str
    ) -> tuple[int, str]:
        if not completion.strip():
            return 0, ""
...
def evaluate_mcqa(
    ...
    batch_size=10,
    verbose=True,
    evaluation_type: MCQAEvaluations = MCQAEvaluations.NEXT_TOKEN,
):
    valid_choices = [
        model.to_single_token(letter) for letter in Question.OPTIONS
    ]

    answers = []
    for i in (
        tqdm(range(0, len(questions), batch_size), desc="Evaluating MCQA")
        if verbose
        else range(0, len(questions), batch_size)
    ):
        prompts = [
            model.wrap_prompt(q.get_prompt(it=model.is_it()))
        ...
        for j, resp in enumerate(logits):
            last_non_pad_idx = (
                (tokens[j] != model.get_pad_token_id()).nonzero()[-1].item()
            )
    ...
    return EvaluationResult(
        ...
        len(questions),
        list(
            zip(str_answers, [question.answer_str for question in questions])
        ),
    )
    ...
    try:
        ...
        for question in questions:
            wrapped_questions.append(
                model.wrap_prompt(
                    f"{prefix}{question.get_prompt(it=model.is_it())}{suffix}"
                )
            )

        only_answers = model.generate_multiple(
            wrapped_questions,
            max_new_tokens=200,
            do_sample=False,
        ...
        if coherence:
            return evaluator.evaluate_open_ended_questions(
                answers, quit_thresh=quit_thresh
            )
        else:
            return evaluator.evaluate_open_ended_questions_no_coherence(
                answers, quit_thresh=quit_thresh
            )
...
def evaluate_mmlu(
    ...
    wrapped_model = (
        TransformerLensModel(model, it=it)
        if isinstance(model, HookedTransformer)
        else TransformersModel(model, tokenizer, it=it)
    )
    return (
        evaluate_mcqa(
            wrapped_model,
            qs if not limit else qs[:limit],
            batch_size=batch_size,
            verbose=verbose,
...
def _copy_mlp_weights(
    model: HookedTransformer, hf_model: AutoModelForCausalLM
):
    for i in range(model.cfg.n_layers):
        hf_model.model.layers[i].mlp.down_proj.weight.data = model.blocks[
            i
        ].mlp.W_out.T.clone()
...
def evaluate_relearning(
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(dataset), batch_size):
            ...
            tokenized_batch["input_ids"] = tokenized_batch["input_ids"].to(
                device
            )
            tokenized_batch["attention_mask"] = tokenized_batch[
                "attention_mask"
            ].to(device)

            outputs = relearned_model(
        ...
        if (epoch + 1) % eval_every == 0:
            with torch.no_grad():
                evals.append(
                    evaluation_function(
                        relearned_model, tokenizer, batch_size, verbose
                    )
                )
...
def evaluate_relearning_full_ft(
    model,
    tokenizer,
    dataset: list[str],
    evaluation_function: Callable,
...
def _get_hf_model_from_tl_model(model: HookedTransformer):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model.cfg.tokenizer_name, device="cuda"
    )
    _copy_mlp_weights(model, hf_model)
    return hf_model, tokenizer


def evaluate_relearning_tl(
...
def evaluate_relearning_full_ft_tl(
    model: HookedTransformer,
    dataset: list[str],
    evaluation_function: Callable,
    batch_size: int,
    ...
    return evaluate_relearning_full_ft(
        hf_model,
        tokenizer,
        dataset,
        evaluation_function,
...
def evaluate_relearning_harry_potter(
    model: AutoModelForCausalLM,
    tokenizer: AutoTokenizer,
    evaluator: AbstractEvaluator,
    limit=2000,
    ...
    return evaluate_relearning(
        ...
        tokenizer,
        ds["train"][:limit]["text"],
        evaluate_harry_potter_open_ended,
        evaluator,
        True,
        hp_limit,
        batch_size=batch_size,
...
def evaluate_relearning_harry_potter_tl(
    model: HookedTransformer,
    evaluator: AbstractEvaluator,
    limit=2000,
    batch_size: int = 10,
    ...
    hf_model, tokenizer = _get_hf_model_from_tl_model(model)
    return evaluate_relearning_harry_potter(
        hf_model, tokenizer, evaluator, limit, batch_size, verbose
    )
...
def eval_alpaca(
    ...
    indices=None,
    verbose: bool = True,
    only_fluency: bool = False,
) -> tuple[list[tuple[int, int]], list[int], list[tuple[str, str, str, str]]]:
    ...
    wrapped_instructions = [
        model.wrap_prompt(instruction) for instruction in instructions
    ]

    if "gemma" in model.tokenizer_name().lower():
        generations = [
            x[x.find("model") + len("model") :]
            for x in model.generate_multiple(
                wrapped_instructions,
    ...
    for instruction, completion in completions:
        ...
        for _ in range(3):
            try:
                if not only_fluency:
                    instruct_score, instruct_response = (
                        evaluator.evaluate_alpaca_instruct_score(
                ...
                fluency_score, fluency_response = (
                    evaluator.evaluate_alpaca_fluency_score(completion + ".")
                )
                break
            except AssertionError as e:
                if _ == 2:  # Last attempt failed
                    raise e
                print(
                    f"Got AssertionError in alpaca evaluation, retrying: {e}"
                )
...
def get_gcg_suffix_tl(
    model: HookedTransformer, questions: list[OpenEndedQuestion], steps=1000
) -> str:
    hf_model, tokenizer = _get_hf_model_from_tl_model(model)
