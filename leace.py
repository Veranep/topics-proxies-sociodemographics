...
class LeaceFitter:
    ...
    def __init__(
        ...
        *,
        ...
        assert (
            svd_tol > 0.0
        ), "`svd_tol` must be positive for numerical stability."
    ...
    def eraser(self) -> LeaceEraser:
        ...
        eye = torch.eye(
            self.x_dim, device=self.mean_x.device, dtype=self.mean_x.dtype
        )
        ...
        if self.constrain_cov_trace and self.method == "leace":
            ...
            if new_trace.real > old_trace.real:
                ...
                alpha = torch.where(alpha1.real > 0, alpha1, alpha2).clamp(
                    0, 1
                )
                P = alpha * P + (1 - alpha) * Q
    ...
    def sigma_xx(self) -> Tensor:
        ...
        if self.shrinkage:
            return optimal_linear_shrinkage(
                S_hat / self.n, self.n, inplace=True
            )
    ...
    def sigma_xz(self) -> Tensor:
        ...
        assert (
            self.n > 1
        ), "Call update() with labels before accessing sigma_xz"
