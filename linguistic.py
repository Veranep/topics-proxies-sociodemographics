...
if __name__ == "__main__":
    ...
    if args.redo_emo_sent:
        ...
        for column in ["user_prompt", "model_response"]:
            ...
            for i in tqdm(range(len(df))):
                ...
                for entry in results[0]:
                    annotations["s_" + entry["label"]].append(entry["score"])
                results = emotion_classifier(text)
                for entry in results[0]:
                    annotations["e_" + entry["label"]].append(entry["score"])
        ...
        if args.dataset == "prism":
            ...
            cluster_ids = cluster_ids.merge(clusters).drop(
                columns=["cluster_id"]
            )
            df = (
            ...
            for i in tqdm(range(len(df))):
                ...
                for sent in spacy_doc.sents:
                    ...
                    for token in sent:
                        if token.is_alpha:
                            num_alpha_tokens += 1
                            if language in ["en", "it", "fr"]:
                                syllable_sum += textstat.syllable_count(
                                    token.text
                                )
                                if (
                                    language == "en"
                ...
                if language in ["en", "it", "fr"]:
                    ...
                    if language in ["en", "fr"]:
                        ...
                        if language == "en":
                            politeness = politeness_classifier(text)[0][
                                "label"
                            ]
                ...
                annotations["num_entities_per_sent"].append(
                    num_entities_per_sent
                )
